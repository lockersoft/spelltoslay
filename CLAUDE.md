# TypeNSpell — Project Notes for Claude Code
...
domain: `typenspell.lockersoft.games` (assumed; confirm with user).
The pattern: clone the repo to `~/typenspell-app/`, symlink
`~/typenspell.lockersoft.games/` → `~/typenspell-app/public/`. No
...
- 1Password entry for the teacher key — the typenspell deploy will need
...
- Profanity check is centralized in `public/api/_bootstrap.php` as
  `slay_is_profane()` — used by `score.php`, `rename.php`, and
...
- Production domain (assume `typenspell.lockersoft.games` until told
  otherwise).
- Whether the SLAY function-name prefix (`slay_*`) gets renamed
  globally or kept as a "shared library" name. Renaming is cleaner;
  keeping is faster.
- Whether to keep the `slay_cid` / `slay_player_name` localStorage keys
