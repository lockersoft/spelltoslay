...
class PlayersTest extends TestCase
{
    ...
    {
        [$status] = slay_invoke('players.php', 'GET', []);
    ...
    public function test_returns_only_fresh_clients_with_names(): void
    {
        ...
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name, personal_message) VALUES ('uuid-fresh', {$now}, 'Dave', '')");
        // Stale client.
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name, personal_message) VALUES ('uuid-stale', " . ($now - 30) . ", 'Stale', '')");

        [$status, , $json] = slay_invoke('players.php', 'GET', ['key' => self::KEY]);
    ...
    {
        $now = time();
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name, personal_message) VALUES ('uuid-pm', {$now}, 'Eve', 'Focus please')");
    ...
    {
        $now = time();
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name, personal_message, personal_paused) VALUES ('uuid-paused', {$now}, 'Frank', '', 1)");
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name, personal_message, personal_paused) VALUES ('uuid-active', {$now}, 'Grace', '', 0)");
    ...
    {
        $now = time();
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name, personal_message, current_score, current_wave, current_hp, is_playing, is_visible)
    ...
    {
        $now = time();
        slay_db()->exec("UPDATE state SET poll_id=5, poll_question='Q?', poll_options='[\"A\",\"B\"]' WHERE id=1");
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name, personal_message) VALUES ('uuid-voter2', {$now}, 'Ivy', '')");
        slay_db()->exec("INSERT INTO poll_responses (poll_id, client_id, option_index, answered_at) VALUES (5, 'uuid-voter2', 0, {$now})");
