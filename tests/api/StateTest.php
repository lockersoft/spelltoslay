...
class StateTest extends TestCase
{
    ...
    {
        [$status, , $json] = sts_invoke('state.php');
    ...
    public function test_etag_returns_304_when_version_matches(): void
    {
        ...
        sts_db()->exec('UPDATE state SET version=5 WHERE id=1');

        [$status, $headers] = sts_invoke('state.php');
        $this->assertSame(200, $status);
        $etag = null;
        foreach ($headers as $h) if (stripos($h, 'ETag:') === 0) $etag = trim(substr($h, 5));
        $this->assertNotNull($etag);

        [$status2] = sts_invoke('state.php', 'GET', [], null, ['If-None-Match' => $etag]);
    ...
    {
        sts_invoke('state.php', 'GET', ['cid' => 'uuid-A']);
        sts_invoke('state.php', 'GET', ['cid' => 'uuid-B']);
        [, , $json] = sts_invoke('state.php', 'GET', ['cid' => 'uuid-A']); // re-poll same client
    ...
    public function test_presence_drops_stale_clients(): void
    {
        ...
        [, , $json] = sts_invoke('state.php', 'GET', ['cid' => 'uuid-A']);
    ...
    {
        sts_invoke('state.php', 'GET', ['cid' => 'uuid-1', 'name' => 'Alice']);
        $row = sts_db()->query("SELECT name FROM presence WHERE client_id='uuid-1'")->fetch();
    ...
    public function test_state_accepts_live_stats(): void
    {
        ...
        $this->assertSame(200, $status);
        $row = sts_db()->query("SELECT current_score, current_wave, current_hp, is_playing, is_visible FROM presence WHERE client_id='uuid-stats'")->fetch();
    ...
    public function test_state_returns_authoritative_name(): void
    {
        ...
        sts_db()->exec("INSERT INTO presence (client_id, last_seen, name) VALUES ('uuid-auth', " . time() . ", 'ServerName')");
        // Poll with a different name.
        [, , $json] = sts_invoke('state.php', 'GET', ['cid' => 'uuid-auth', 'name' => 'ClientName']);
        $this->assertSame('ServerName', $json['name']);
        // DB should still have ServerName.
        $row = sts_db()->query("SELECT name FROM presence WHERE client_id='uuid-auth'")->fetch();
    ...
    public function test_state_first_name_set_when_empty(): void
    {
        ...
        sts_invoke('state.php', 'GET', ['cid' => 'uuid-fresh2', 'name' => 'NewPlayer']);
        $row = sts_db()->query("SELECT name FROM presence WHERE client_id='uuid-fresh2'")->fetch();
        $this->assertSame('NewPlayer', $row['name']);
    }

    public function test_state_includes_poll_when_active(): void
    {
        sts_db()->exec("UPDATE state SET poll_id=1, poll_question='Who is the best?', poll_options='[\"Alice\",\"Bob\"]' WHERE id=1");
        [, , $json] = sts_invoke('state.php', 'GET', ['cid' => 'uuid-poll1']);
    ...
    {
        sts_db()->exec("UPDATE state SET poll_id=2, poll_question='Favourite?', poll_options='[\"Cat\",\"Dog\"]' WHERE id=1");
        sts_db()->exec("INSERT INTO poll_responses (poll_id, client_id, option_index, answered_at) VALUES (2, 'uuid-voter', 1, " . time() . ")");
