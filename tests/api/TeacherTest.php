...
class TeacherTest extends TestCase
{
    ...
    {
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name, personal_message) VALUES ('uuid-Y', " . time() . ", 'Charlie', '')");
        [$status, , $json] = slay_invoke('teacher.php', 'POST', ['key' => self::KEY], [
            'action' => 'messageStudent', 'cid' => 'uuid-Y', 'text' => 'Stop talking',
        ]);
        $this->assertSame(200, $status);
        $this->assertTrue($json['ok']);

        $row = slay_db()->query("SELECT personal_message FROM presence WHERE client_id='uuid-Y'")->fetch();
        $this->assertSame('Stop talking', $row['personal_message']);

        $stateRow = slay_db()->query('SELECT version FROM state WHERE id=1')->fetch();
        $this->assertSame(1, (int)$stateRow['version']);
    }

    public function test_message_student_truncates_at_200(): void
    {
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, personal_message) VALUES ('uuid-Z', " . time() . ", '')");
        $long = str_repeat('x', 250);
        slay_invoke('teacher.php', 'POST', ['key' => self::KEY], [
            'action' => 'messageStudent', 'cid' => 'uuid-Z', 'text' => $long,
        ]);
        $row = slay_db()->query("SELECT personal_message FROM presence WHERE client_id='uuid-Z'")->fetch();
        $this->assertSame(200, mb_strlen($row['personal_message']));
    }

    public function test_message_student_empty_text_clears(): void
    {
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, personal_message) VALUES ('uuid-W', " . time() . ", 'some old message')");
        slay_invoke('teacher.php', 'POST', ['key' => self::KEY], [
            'action' => 'messageStudent', 'cid' => 'uuid-W', 'text' => '',
        ]);
        $row = slay_db()->query("SELECT personal_message FROM presence WHERE client_id='uuid-W'")->fetch();
    ...
    {
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name, personal_message, personal_paused) VALUES ('uuid-PS', " . time() . ", 'Dave', '', 0)");
        [$status, , $json] = slay_invoke('teacher.php', 'POST', ['key' => self::KEY], [
        ...
        $row = slay_db()->query("SELECT personal_paused FROM presence WHERE client_id='uuid-PS'")->fetch();
    ...
    {
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name, personal_message, personal_paused) VALUES ('uuid-RS', " . time() . ", 'Eve', '', 1)");
        slay_db()->exec('UPDATE state SET version=3 WHERE id=1');
        [$status, , $json] = slay_invoke('teacher.php', 'POST', ['key' => self::KEY], [
        ...
        $row = slay_db()->query("SELECT personal_paused FROM presence WHERE client_id='uuid-RS'")->fetch();
    ...
    {
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name) VALUES ('uuid-rn', " . time() . ", 'OldName')");
        [$status, , $json] = slay_invoke('teacher.php', 'POST', ['key' => self::KEY], [
            'action' => 'renameStudent', 'cid' => 'uuid-rn', 'name' => 'NewName',
        ]);
        $this->assertSame(200, $status);
        $this->assertTrue($json['ok']);
        $row = slay_db()->query("SELECT name FROM presence WHERE client_id='uuid-rn'")->fetch();
    ...
    {
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name) VALUES ('uuid-rnv', " . time() . ", 'Someone')");
        [$status] = slay_invoke('teacher.php', 'POST', ['key' => self::KEY], [
    ...
    {
        slay_db()->exec("INSERT INTO presence (client_id, last_seen, name) VALUES ('uuid-rnp', " . time() . ", 'Good')");
        [$status] = slay_invoke('teacher.php', 'POST', ['key' => self::KEY], [
    ...
    public function test_start_poll_sets_state(): void
    {
        ...
        $this->assertSame(200, $status);
        $this->assertTrue($json['ok']);
        $row = slay_db()->query('SELECT poll_id, poll_question, poll_options FROM state WHERE id=1')->fetch();
    ...
    {
        slay_db()->exec("UPDATE state SET poll_id=1, poll_question='Running poll', poll_options='[\"A\",\"B\"]' WHERE id=1");
        [$status] = slay_invoke('teacher.php', 'POST', ['key' => self::KEY], ['action' => 'endPoll']);
        $this->assertSame(200, $status);
        $row = slay_db()->query('SELECT poll_question FROM state WHERE id=1')->fetch();
